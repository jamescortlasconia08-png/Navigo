...
import {
    ...
    Search, SlidersHorizontal, Star, Eye, Plane, Building, Camera, Car, Share2, Download
} from 'lucide-react';
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden transform hover:-translate-y-1 transition-transform duration-300">
            <div className="relative">
                ...
                <div className="absolute top-3 left-3 bg-cyan-600 text-white text-xs font-bold px-2 py-1 rounded-full">{trip.type}</div>
                <div className="absolute top-3 right-3 flex space-x-2">
                    {trip.isFavorite && <button className="w-8 h-8 rounded-full flex items-center justify-center bg-yellow-400/80 backdrop-blur-sm text-white"><Star size={18} fill="currentColor" /></button>}
                    <button className="w-8 h-8 rounded-full flex items-center justify-center bg-white/80 backdrop-blur-sm text-gray-700"><Eye size={18} /></button>
                </div>
                <div className="absolute bottom-3 left-3 flex items-center gap-1">
                    {[...Array(5)].map((_, i) => <Star key={i} size={16} className={i < trip.rating ? 'text-yellow-400' : 'text-gray-300'} fill="currentColor" />)}
                </div>
            </div>
            <div className="p-5">
                <h3 className="text-xl font-bold truncate">{trip.destination}</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">{trip.dates}</p>
                <p className="text-sm text-gray-600 dark:text-gray-300 italic mb-4 h-10">"{trip.description}"</p>
                ...
                <div className="flex justify-around items-center text-center text-xs text-gray-500 dark:text-gray-400 py-3 border-t border-b border-gray-200 dark:border-gray-700 mb-4">
                    <div className="flex items-center gap-1"><Plane size={14}/><span>{trip.activities.flights}</span></div>
                    <div className="flex items-center gap-1"><Building size={14}/><span>{trip.activities.hotels}</span></div>
                    <div className="flex items-center gap-1"><Camera size={14}/><span>{trip.activities.activities}</span></div>
                    <div className="flex items-center gap-1"><Car size={14}/><span>{trip.activities.transport}</span></div>
                </div>
                ...
                <div className="flex gap-2">
                    ...
                    <button className="p-2 bg-gray-200 dark:bg-gray-700 rounded-lg"><Download size={16}/></button>
                </div>
            </div>
        </div>
    );
...
const History = () => {
    ...
    const filteredTrips = tripHistoryData.filter(trip => 
            ...
            <header className="bg-white dark:bg-gray-800 shadow-sm sticky top-0 z-50 flex items-center justify-between px-6 py-3">
                <div className="flex items-center space-x-6">
                    ...
                    <nav className="hidden md:flex items-center space-x-4">
                        ...
                        <a href="#" className="flex items-center space-x-2 text-cyan-600 font-semibold border-b-2 border-cyan-600 pb-1"><Clock size={20} /><span>History</span></a>
            ...
            <main className="p-6 lg:p-10">
                <div className="max-w-7xl mx-auto">
                    <div className="mb-8">
                        ...
                        <p className="text-gray-500 dark:text-gray-400 mt-2">Your complete travel journey with memories, expenses, and insights.</p>
                    </div>
                    ...
                    <div className="mb-8 p-4 bg-white dark:bg-gray-800 rounded-xl shadow-md">
                        <div className="flex flex-col md:flex-row gap-4">
                            ...
                            <div className="flex gap-2">
                                <button className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-3 bg-gray-100 dark:bg-gray-700 rounded-lg font-semibold text-sm"><ChevronDown size={16} /><span>All Years</span></button>
                                <button className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-3 bg-gray-100 dark:bg-gray-700 rounded-lg font-semibold text-sm"><ChevronDown size={16} /><span>All Types</span></button>
                                <button className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-3 bg-gray-100 dark:bg-gray-700 rounded-lg font-semibold text-sm"><SlidersHorizontal size={16} /><span>More</span></button>
                            </div>
                        </div>
                    </div>
